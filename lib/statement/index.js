...
const getCharges = csvPath => {
    // todo Fix this! groupBy only returns sub-arrays with multiple elements if they are already located together
    // todo Use 'duplicates' and 'unique' in misc module for this
    ...
    // Partition off any duplicates and return an array of 2 arrays.
    // The first should contain any duplicates and the second containing all the rest.
    return S.map (outer => {
        console.log(`outer: ${JSON.stringify(outer)}`);
        const duplicates = S.map(arr => arr[0])(S.filter (inner => inner.length > 1) (outer));
