...
const duplicates = input => equal => {
    const loop = input => acc => {
        const dups = (head, tail) => {
            ...
            // If there are more than 2, then wait until the next is encountered before returning all
            return filtered.length === 0 ? [] : filtered.length === 1 ? S.append (head) (filtered) : [head];
        };
        const [head, ...tail] = input;
        return head ? loop (tail) (S.concat (acc) (dups (head, tail))) : acc;
    };
